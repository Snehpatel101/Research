...
This document is preserved for historical context and may not match the current codebase.

For the current architecture reference, see `docs/reference/ARCHITECTURE.md`.
...
│              (Data)      (Models)    (CV/OOF)   (Ensemble)  (Inference/Monitoring) │
│                                                                                      │
│  Status:     COMPLETE    COMPLETE    COMPLETE   COMPLETE    PARTIAL                 │
└─────────────────────────────────────────────────────────────────────────────────────┘
```

## Symbol Isolation (“Single-Contract”) Architecture

**Critical Design Decision: Each contract trains in complete isolation.**

### Key Principles

1. **Isolation by default** - Each symbol’s data is processed independently (no cross-symbol features/labels)
2. **Batch is explicit** - The CLI blocks multi-symbol runs unless you opt in (`allow_batch_symbols`)
3. **Easy switching** - Symbol selection is a first-class config parameter
4. **Separate models per symbol** - MES model artifacts are not reused for MGC (unless you explicitly do so)
...
│
└── tests/                           # Unit + integration tests (see pytest config)
    ├── models/                      # Model tests
