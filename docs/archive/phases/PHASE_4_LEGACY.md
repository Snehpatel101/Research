...
Phase 4 is implemented as part of the Phase 2 “model factory” via the ensemble model family. Ensembles are trained through the same `Trainer` entrypoint and config system as single models.

**Code:**
- Models: `src/models/ensemble/voting.py`, `src/models/ensemble/stacking.py`, `src/models/ensemble/blending.py`
- Registry + trainer: `src/models/registry.py`, `src/models/trainer.py`
- Configs: `config/models/voting.yaml`, `config/models/stacking.yaml`, `config/models/blending.yaml`
...
> Note: Ensemble configs determine base models (and meta-learner for stacking/blending). See the YAMLs in `config/models/`.

---

## What “ensemble” means in this repo

- **Voting**: combines base model probabilities/predictions by soft/hard voting.
- **Stacking**: trains a meta-learner on base model predictions (often using out-of-fold predictions from Phase 3).
- **Blending**: similar to stacking but uses a holdout blend set rather than full OOF.

If you want *strict* stacking hygiene (meta-learner trained only on OOF predictions), use Phase 3 to generate OOF predictions first and then follow the ensemble workflow documented in `docs/phases/PHASE_3.md`.
