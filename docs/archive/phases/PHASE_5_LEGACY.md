...
Phase 5 is **not** a single “one command” deployment flow yet, but the core building blocks exist:

**Implemented:**
- Inference pipeline and bundle format: `src/inference/pipeline.py`, `src/inference/bundle.py`
- Batch inference helper: `src/inference/batch.py`, `scripts/batch_inference.py`
- HTTP serving (Flask, optional extra): `src/inference/server.py`, `scripts/serve_model.py`
- Drift detection primitives: `src/monitoring/drift_detector.py`, `src/monitoring/alert_handler.py`

**Not implemented / not wired end-to-end yet:**
- A single “evaluate final test set” script (docs previously referenced `scripts/evaluate_test.py`, which does not exist)
- A standard “train → bundle → serve” pipeline step (training currently writes to `experiments/runs/...`; bundles are a separate artifact format)
- A formal “test-set integrity audit log” mechanism (hash/access auditing)

---

## Serving a model (requires a bundle directory)

The serving and batch inference CLIs operate on **bundle directories**:
...
Bundle structure is defined by `src/inference/bundle.py` (metadata + features + optional scaler/calibrator + model artifacts).

---

## Monitoring (drift detection)

Drift detection is implemented as reusable primitives (not a production daemon):
- Detectors and thresholds: `src/monitoring/drift_detector.py`
- Alert routing/rate limiting: `src/monitoring/alert_handler.py`

If you want this to run continuously, the missing piece is an application wrapper (e.g., ingest features/predictions, call detectors, emit alerts).
