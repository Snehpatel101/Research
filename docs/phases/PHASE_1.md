...
Stage 3: Features        150+ technical indicators
    |
Stage 4: MTF             Multi-timeframe indicator features (15m, 30m, 1h, 4h, daily)
    |
...
| **MTF** | Base features | +30 MTF indicators | 5 of 9 timeframes: 15m, 30m, 1h, 4h, daily (intended: 1m, 5m, 10m, 15m, 20m, 25m, 30m, 45m, 1h) |
...
### Current Status (Strategy 2 - Partial)

**Location:** `src/phase1/stages/mtf/`

**What's Implemented:**
- MTF indicator generation from **5 of 9 intended timeframes** (15min, 30min, 1h, 4h, daily)
- Anti-lookahead protection via `shift(1)` before forward-fill alignment
- ~30 MTF indicator features added to ~150 base features = ~180 total
- **Missing 4 timeframes:** 1min, 10min, 20min, 25min (part of intended 9-timeframe ladder)
...
class MTFMode(str, Enum):
    ...
    BOTH = 'both'           # Both OHLCV + indicators (DEFAULT)
```

**Current Default:** `MTFMode.BOTH` → All models receive OHLCV + indicators from all timeframes

###  What's Missing (From MTF_IMPLEMENTATION_ROADMAP.md)

| Strategy | Data Type | Intended Models | Status |
|----------|-----------|-----------------|--------|
| **Strategy 1: Single-TF** | One timeframe, no MTF | All 19 models (baselines) | ❌ Not implemented |
| **Strategy 2: MTF Indicators** | Indicator features from 9 timeframes | **Tabular (6):** Boosting (XGBoost, LightGBM, CatBoost) + Classical (RF, Logistic, SVM) | ⚠️ Partial (5 of 9 TFs, all models get this) |
| **Strategy 3: MTF Ingestion** | Raw OHLCV bars from 9 timeframes as multi-resolution tensors | **Sequence (13):** Neural (LSTM, GRU, TCN, Transformer) + CNN (InceptionTime, 1D ResNet) + Advanced (PatchTST, iTransformer, TFT) + MLP (N-BEATS) | ❌ Not implemented |

**9-Timeframe Ladder (Intended):** 1min, 5min, 10min, 15min, 20min, 25min, 30min, 45min, 1h

**Critical Limitation:**
All features generated by Phase 1 are **indicator-derived**. Sequence models receive pre-computed indicators when they should ideally receive **raw multi-resolution OHLCV bars** for temporal learning.

**Example of Missing Functionality (Strategy 3):**
```python
# This doesn't exist yet:
multi_resolution_data = {
    '5min':  torch.Tensor(n_samples, 60, 4),    # 60 bars, raw OHLCV
    '15min': torch.Tensor(n_samples, 20, 4),    # 20 bars, raw OHLCV
    '1h':    torch.Tensor(n_samples, 5, 4),     # 5 bars, raw OHLCV
}
# Models like TFT, PatchTST, TimesNet could process these jointly
```

**Impact on Model Performance:**
- **Tabular models (6):** Boosting + Classical → Receive appropriate indicator features ✅ (but only 5 of 9 timeframes)
- **Sequence models (13):** Neural + CNN + Advanced + MLP → Receive indicators instead of raw bars ⚠️ Suboptimal for temporal learning

**Current Sequence Models (7):** LSTM, GRU, TCN, Transformer
**Planned Sequence Models (6):** InceptionTime, 1D ResNet, PatchTST, iTransformer, TFT, N-BEATS

**Future Work:** See `docs/roadmaps/MTF_IMPLEMENTATION_ROADMAP.md` (6-8 weeks implementation)
