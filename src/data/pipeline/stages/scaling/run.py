...
Pipeline wrapper for train-only feature scaling with multi-timeframe support.

PIPE-001/PIPE-002 Fix: This stage now handles multi-TF mode by processing
each timeframe's splits separately, maintaining per-TF scalers.
"""
...
def _scale_single_timeframe(
    ...
    """
    Scale a single timeframe's data.
    ...
    Returns:
        Tuple of (artifacts list, scaling metadata)
    """
    logger.info(f"\n{'='*50}")
    logger.info(f"Scaling timeframe: {tf}")
    logger.info(f"{'='*50}")

    # Determine paths based on multi-TF mode
    if is_multi_tf:
        combined_path = config.final_data_dir / f"combined_{tf}_final_labeled.parquet"
        splits_dir = config.splits_dir / tf
        scaled_data_dir = config.splits_dir / tf / "scaled"
    else:
    ...
    if not combined_path.exists():
        raise FileNotFoundError(f"Combined data not found for {tf}: {combined_path}")

    df = pd.read_parquet(combined_path)
    logger.info(f"Loaded combined data for {tf}: {len(df):,} rows")
    ...
    if len(feature_cols) == 0:
        raise ValueError(f"No feature columns identified for scaling in {tf}")
    ...
    if leakage_check["leakage_detected"]:
        raise RuntimeError(
            f"DATA LEAKAGE DETECTED in scaling for {tf}! Issues: {leakage_check['issues']}"
        )
    ...
    logger.info("\n" + "-" * 50)
    logger.info(f"SCALING SUMMARY for {tf}")
    logger.info("-" * 50)
    ...
    for artifact_path in artifacts:
        manifest.add_artifact(
            name=f"scaling_{tf}_{artifact_path.name}",
            file_path=artifact_path,
            stage="feature_scaling",
            metadata=scaling_metadata,
        )

    return artifacts, scaling_metadata
...
def run_feature_scaling(config: "PipelineConfig", manifest: "ArtifactManifest") -> StageResult:
    ...
    1. Loads train/val/test splits (per-timeframe in multi-TF mode)
