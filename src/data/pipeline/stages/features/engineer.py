...
class FeatureEngineer:
    ...
    def engineer_features(self, df: pd.DataFrame, symbol: str) -> tuple[pd.DataFrame, dict]:
        ...
        if self.enable_wavelets and len(df) >= self.wavelet_window:
            try:
                ...
                wavelet_cols_added = len([c for c in df.columns if c.startswith("wavelet_")])
        ...
        df, nan_audit = clean_nan_columns(df, symbol=symbol, nan_threshold=self.nan_threshold)
        ...
        logger.info(
            f"Columns: {initial_cols} -> {len(df.columns)} (+{feature_report['features_added']} features, -{cols_dropped} dropped for NaN)"
        )
        ...
        try:
            ...
            logger.info(f"Feature cache saved: {cache_key}")
        except Exception as e:
    ...
    def save_features(
        ...
        # Save metadata
        metadata = {"report": feature_report, "features": self.feature_metadata}
    ...
    def process_multi_symbol(self, symbol_files: dict[str, str | Path]) -> dict[str, dict]:
        """
