...
def validate_feature_schema(
    ...
    *,
    ...
    if missing_by_file or extra_by_file:
        ...
        error_lines.append("Check upstream stages (features, scaling) for consistency issues.")
...
def run_build_datasets(config: "PipelineConfig", manifest: "ArtifactManifest") -> StageResult:
    ...
    try:
        ...
        if is_multi_tf and len(output_timeframes) > 1:
            ...
            for tf in output_timeframes:
                ...
                if tf_train_path.exists():
                    ...
                    logger.warning(
                        f"Skipping {tf} in cross-TF validation: {tf_train_path} not found"
                    )
        ...
        # Summary
        total_datasets = (
            len(feature_set_names) * len(config.label_horizons) * len(output_timeframes)
        )
