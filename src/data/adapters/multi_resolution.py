...
class MultiResolution4DConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for tf in self.timeframes:
            if tf not in TIMEFRAME_TO_MINUTES and tf not in TIMEFRAME_SUFFIX_PATTERNS:
                raise ValueError(
                    f"Unknown timeframe '{tf}'. Valid options: {list(TIMEFRAME_TO_MINUTES.keys())}"
                )
...
class MultiResolution4DAdapter:
    ...
    def __init__(
        ...
        Args:
            timeframes: List of timeframes to include. Default: 7-timeframe ladder
...
def create_multi_resolution_dataset(
    ...
    Args:
        ...
        timeframes: List of timeframes (default: 7-timeframe ladder)
        seq_len: Sequence length
        stride: Step between sequences
        label_column: Label column name
