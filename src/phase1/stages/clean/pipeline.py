...
def clean_symbol_data(
    ...
    2. OHLC validation
    3. Gap detection and filling (missing_bar flag)
    4. Resampling to target timeframe (configurable, defaults to 5min)
    5. Add futures/session metadata flags (roll window, session_id)
    6. Final OHLC validation
    ...
    include_session_metadata : bool
        If True, adds a session_id column based on session definitions.
    roll_gap_threshold : float
