...
def is_cme_market_closed(
    dt: pd.Timestamp, tz: str = "America/Chicago", assume_utc: bool = True
) -> bool:
    """
...
class GapHandler:
    ...
    def detect_gaps(self, df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
        ...
        gap_report = {
            ...
            "completeness_pct": (
                (total_actual_bars / total_expected_bars * 100) if total_expected_bars > 0 else 100
            ),
        ...
        logger.info(
            f"Found {len(gaps)} total gaps ({expected_gap_count} expected, {len(unexpected_gaps)} unexpected)"
        )
    ...
    def fill_gaps(self, df: pd.DataFrame, max_fill_bars: int | None = None) -> pd.DataFrame:
        ...
        date_range = pd.date_range(
            start=df["datetime"].min(), end=df["datetime"].max(), freq=f"{self.freq_minutes}min"
        )
        ...
        if n_filled > 0:
            ...
            if self.gap_fill_method == "forward":
                ...
                df_complete[["open", "high", "low", "close"]] = df_complete[
                    ["open", "high", "low", "close"]
                ].ffill(limit=max_fill_bars)
                ...
                df_complete[["open", "high", "low", "close"]] = df_complete[
                    ["open", "high", "low", "close"]
                ].interpolate(method="linear", limit=max_fill_bars)
                df_complete["volume"] = df_complete["volume"].fillna(0)
...
def create_gap_handler(
    ...
    if gap_fill_method not in valid_methods:
        raise ValueError(
            f"Invalid gap_fill_method: '{gap_fill_method}'. " f"Must be one of: {valid_methods}"
        )
