...
def add_historical_volatility(
    ...
    for period in periods:
        ...
        df[f'hvol_{period}'] = (log_returns.rolling(window=period).std() * annualization_factor).shift(1)
...
def add_parkinson_volatility(
    ...
    parkinson_raw = np.sqrt((1 / (4 * np.log(2))) *
...
def add_rogers_satchell_volatility(
    ...
    rs_component = (log_high - log_close) * (log_high - log_open) + \
