...
def get_labeling_strategy_config(strategy: LabelingStrategyType | str) -> dict:
    """
...
def validate_labeling_config() -> list[str]:
    ...
    for strategy_type, config in LABELING_STRATEGY_CONFIGS.items():
        if not isinstance(config, dict):
            errors.append(f"LABELING_STRATEGY_CONFIGS[{strategy_type}] must be a dict")
    ...
    if not (0 <= winsorize < 0.5):
        errors.append(f"REGRESSION strategy 'winsorize_pct' must be in [0, 0.5), got {winsorize}")
