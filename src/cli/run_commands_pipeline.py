...
def run_command(
    ...
    run_id: str | None = typer.Option(
        None, "--run-id", help="Custom run ID (defaults to YYYYMMDD_HHMMSS)"
    ),
    description: str | None = typer.Option(
        None, "--description", "-d", help="Description of this run"
    ),
    train_ratio: float | None = typer.Option(
        None, "--train-ratio", help="Training set ratio (default: 0.70)"
    ),
    val_ratio: float | None = typer.Option(
        None, "--val-ratio", help="Validation set ratio (default: 0.15)"
    ),
    test_ratio: float | None = typer.Option(
        None, "--test-ratio", help="Test set ratio (default: 0.15)"
    ),
    purge_bars: int | None = typer.Option(
        None, "--purge-bars", help="Number of bars to purge at split boundaries"
    ),
    embargo_bars: int | None = typer.Option(
        None, "--embargo-bars", help="Embargo period in bars (~1 day for 5-min data)"
    ),
    horizons: str | None = typer.Option(
        None, "--horizons", help="Comma-separated label horizons (overrides preset)"
    ),
    ...
    mtf_mode: str | None = typer.Option(
        None, "--mtf-mode", help="MTF mode: bars, indicators, or both (default: both)"
    ),
    ...
    meta_learner: str | None = typer.Option(
        None, "--meta-learner", help="Meta-learner for ensemble stacking (e.g., logistic, xgboost)"
    ),
    sequence_length: int | None = typer.Option(
        None, "--sequence-length", help="Sequence length for sequential models (LSTM, Transformer)"
    ),
...
def rerun_command(
    ...
    from_stage: str | None = typer.Option(
        None, "--from", help="Stage to resume from (e.g., 'labels', 'features')"
    ),
    project_root: str | None = typer.Option(None, "--project-root", help="Project root directory"),
) -> None:
    """
