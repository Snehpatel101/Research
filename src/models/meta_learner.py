...
from src.pipeline.stages.labeling.meta import MetaLabeler, BetSizeMethod
...
class MetaLearner:
    ...
    def train(
        ...
        if hasattr(self.primary_model, "predict_proba"):
            ...
            confidence = np.max(primary_probs, axis=1)
        else:
            primary_preds = self.primary_model.predict(X_train)
            confidence = np.ones_like(primary_preds, dtype=float)
