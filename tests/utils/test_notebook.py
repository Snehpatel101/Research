...
class TestColabSetup:
    """Tests for colab_setup.py module."""

    def test_is_colab_returns_bool(self):
        """is_colab should return boolean."""
        import sys
        sys.path.insert(0, "/home/jake/Desktop/Research")
        from colab_setup import is_colab

        result = is_colab()
        assert isinstance(result, bool)

    def test_is_kaggle_returns_bool(self):
        """is_kaggle should return boolean."""
        from colab_setup import is_kaggle

        result = is_kaggle()
        assert isinstance(result, bool)

    def test_get_gpu_info_returns_dict(self):
        """get_gpu_info should return info dict."""
        from colab_setup import get_gpu_info
    ...
    def test_setup_returns_results(self):
        """setup should return results dict."""
        from colab_setup import setup

        with patch("colab_setup.install_packages") as mock_install:
            ...
            assert isinstance(result, dict)
            assert "is_colab" in result
            assert "is_kaggle" in result
            assert "gpu" in result

    def test_install_packages_returns_dict(self):
        """install_packages should return success dict."""
        from colab_setup import install_packages

        with patch("subprocess.check_call") as mock_call:
            mock_call.return_value = 0
            result = install_packages(["numpy"], quiet=True)

            assert isinstance(result, dict)

    def test_quick_test_function_exists(self):
        """quick_test function should exist."""
        from colab_setup import quick_test
