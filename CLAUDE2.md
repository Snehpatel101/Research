...
This file provides operating instructions for Claude Code when working in this repository. Treat it as the repo’s **constitution**: adhere to the contracts, invariants, and engineering rules below.

## North Star: OHLCV ML Factory for Single-Contract Futures

We are building a **production-grade, model-agnostic ML factory** for systematic futures trading research *and* deployment.

**Input:** one canonical OHLCV dataset for a single contract (e.g., MES or MGC)  
**Output:** reproducible, leakage-safe training datasets; trained model artifacts; evaluation reports; inference-ready predictors.

### What “factory” means here

- **One data source → many derived datasets:** deterministic resampling and rational **multi-timeframe (MTF)** feature generation.
- **Model-family specific adapters (not one-size-fits-all):** tabular vs sequence models require different representations, feature formulations, scaling, and windowing — *without changing the underlying data or evaluation protocol*.
- **Fair comparison:** same splits, same leakage prevention, same metrics/backtest assumptions; only the model changes.
- **Research → inference parity:** the exact same preprocessing graph is used at training time and at runtime.

### Inference & deployment are first-class

Every trained model must ship with:
- a frozen **feature/label schema** and versioned config,
- a deterministic **preprocessing + resampling** pipeline,
- a `predict()` output that is stable and machine-consumable (probabilities/expected returns + confidence),
- optional calibration / thresholding hooks for trading decisions.

### Meta-learning & stacking

Ensembles are not just averaging. We support:
- **OOF-based stacking** where base models generate out-of-fold predictions under time-series-aware CV,
- a **meta-learner** trained only on OOF predictions (no leakage) to learn *when to trust which model* (including regime sensitivity),
- strict compatibility: **tabular-only stacks** and **sequence-only stacks** (no mixed input shapes).

### Non-negotiable invariants

- **Single-contract isolation:** no cross-symbol correlation, features, or labels — ever.
- **Leakage paranoia:** purge/embargo, train-only transforms, fold-local feature selection.
- **Modular, testable code:** small files, explicit boundaries, delete dead code.
...
### Representation Adapters (Per Model Family)

We do **not** force a single universal feature matrix onto every architecture. The factory produces family-appropriate datasets from the same underlying OHLCV stream:

- **Tabular models (2D input):** engineered feature matrices designed for boosting/classical learners (stationarity-aware transforms, explicit feature selection, robust scaling).
- **Sequence models (3D input):** windowed tensors with temporal context for PatchTST/TCN/RNN/Transformer-style learners (sequence length, stride, per-window normalization).
- **Shared invariants:** identical labels, identical time splits, identical leakage controls — only the *representation adapter* changes.

### Train/Serve Parity (Inference is a Contract)

Research artifacts must be deployable without rewriting preprocessing:

- The **exact same resampling + feature pipeline** used in training must be used in inference.
- Every run must emit a versioned **artifact bundle** (config + schema + preprocessing graph + model weights) and a thin inference wrapper that produces stable `PredictionOutput`.
...
The factory supports both **single models** and **ensembles**, but ensembles must remain **leakage-safe** and schema-consistent.

- **Voting / blending:** combine base-model predictions directly (optionally weighted).
- **Stacking (meta-learning):** train a meta-learner on **out-of-fold (OOF)** base-model predictions generated under time-series-aware CV. The meta-learner must never see in-fold predictions from models trained on overlapping future data.
