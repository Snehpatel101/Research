...
def main():
    ...
    print(
        f"Train indices: {len(train_indices):,} (range: {train_indices.min()}-{train_indices.max()})"
    )
    print(f"Val indices:   {len(val_indices):,} (range: {val_indices.min()}-{val_indices.max()})")
    print(
        f"Test indices:  {len(test_indices):,} (range: {test_indices.min()}-{test_indices.max()})"
    )
    ...
    print(
        f"  Val:   {len(val_df):,} -> {len(val_scaled):,} ({(1-len(val_scaled)/len(val_df))*100:.1f}% loss)"
    )
    print(
