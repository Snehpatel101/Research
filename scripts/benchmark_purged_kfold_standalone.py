"""
Standalone benchmark script for PurgedKFold vectorized optimization.

This script directly uses the purged_kfold.py implementation without importing
the full cross_validation module to avoid dependency issues.
...
def benchmark_single_size(
    ...
    """
    logger.info(f"Benchmarking {n_samples:,} samples...")
...
def main() -> None:
    ...
    # Default sizes: small, medium, large, extra large
    sizes = [1000, 10000, 50000, 100000]

    # Run benchmarks
    results = run_benchmarks(sizes, n_splits=5, verify=True)

    # Print summary
    print_summary(results)

    # Exit with error if results don't match
    if not all(r.identical_results for r in results):
